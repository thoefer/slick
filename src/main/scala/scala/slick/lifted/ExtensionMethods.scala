...
final class AnyOptionExtensionMethods[O, P](val r: O) extends AnyVal {
  def map[Q, QO](f: P => Q)(implicit ol: OptionLift[Q, QO]): QO = ???
  def get: P = ???
}
...
trait ExtensionMethodConversionsLowPriority {
  implicit def anyOptionOtherExtensionMethods[T](v: Rep[Option[T]]) = new AnyOptionExtensionMethods[Rep[Option[T]], Rep[T]](v)
}
